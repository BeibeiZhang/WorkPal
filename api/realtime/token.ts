      ...
      body: JSON.stringify({
        ...
        tools: [
          ...
          },
          {
            ...
            name: 'web_search',
            description:
              'Search the live web for current facts — prices, news, product specs, official-site content, anything that may have changed since training. ALWAYS call this for price/fact questions; do not refuse. Speak a short one-sentence lead-in BEFORE calling so the user hears that you are looking it up, then speak a concise synthesized answer after the tool returns. The UI shows source chips and product images automatically.',
            parameters: {
              type: 'object',
              properties: {
                query: {
                  type: 'string',
                  description: 'Specific search query. Include product name, brand, and market/country, and match the language appropriate to the target site (Chinese for chanel.cn, English for chanel.com).',
                },
                max_results: {
                  type: 'number',
