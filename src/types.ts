...
/** Category tag for an output artifact (whether generated by Claude Code
 *  locally or produced by the hosted artifact primitive). Drives the icon +
 *  the Project Output filter chips. 'File' is the catch-all for anything that
 *  doesn't fit the three visual media types. */
export type OutputType = 'Web' | 'Slides' | 'Image' | 'Video' | 'File';

/** Row in a project's Output grid. Persisted onto `Project.outputs` — the
 *  icon is derived from `type` at render time (see `outputIconFor` in
 *  shared.tsx) so this shape stays JSON-serializable for localStorage. */
export interface OutputItem {
  id: string;
  name: string;
  type: OutputType;
  /** Candidate #3: set for hosted artifacts (ArtifactRef.source='artifact').
   *  Clicking the Output card opens this URL (typically `/artifact/<slug>`)
 ...
 *  bubble as a small clickable card. Also the shape #3's artifact primitive
 *  will push when a hosted digest is generated — `source` disambiguates. */
export interface ArtifactRef {
  /** Filename or title shown in the card ("claude-intro.html"). */
  name: string;
  fileType: OutputType;
  /** Absolute file path — present for `source: 'claude-code'` so the
   *  backend `open-file` endpoint can re-open the artifact. */
  path?: string;
  /** Click target. Claude-code files leave this undefined and rely on
   *  `path`; hosted artifacts (#3) populate this with `/artifact/<slug>`. */
  href?: string;
  source: 'claude-code' | 'artifact' | 'upload';
}
...
export interface Message {
  ...
  /** Files produced by the assistant in this turn (currently via Claude Code
