...
import { getOAuthClient, type GoogleConnectorId } from './googleAuth.js';

/** Minimal card-shape snapshots that mirror src/types.ts. These leave the
 *  server as plain JSON and are rendered by MessageCard on the client, so the
...
/** Successful tool result returned to the LLM for the second pass. Wraps a
 *  UI card + a compact, model-friendly JSON summary so the model can write
 *  a text synthesis referring to specific items. */
...
/** Encode a plain RFC 2822 message as base64url (Gmail API requires it). */
function encodeMessage(raw: string): string {
  return Buffer.from(raw, 'utf8').toString('base64')
    .replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}

/** Normalise a datetime string into RFC 3339 with a timezone — Google Calendar's
 *  `timeMin` / `timeMax` / `start.dateTime` fields require one, but LLMs often
 *  emit bare "2026-04-20T15:00:00". Date-only ("2026-04-20") becomes midnight UTC. */
...
export async function sendEmail(args: SendEmailArgs): Promise<ToolResult> {
  ...
  // Gmail-branded card: subject as bold title, body preview below, sender
  // footer. Mirrors the reference design — no status pill / action buttons.
...
export async function listCalendarEvents(args: ListEventsArgs): Promise<ToolResult> {
  ...
  // Default to now + next 7 days so generic "what's on my calendar" calls
  ...
  // Use ScheduleCard so the client renders the simple "icon + title + info rows"
  // layout. Header uses the Gmail icon per the reference — these events typically
  // arrived via a Gmail invite, so the Gmail logo feels natural as the source.
  // Shows the soonest event; the text synthesis (for empty result only) covers
  // the "anything more?" case.
...
export async function createCalendarEvent(args: CreateEventArgs): Promise<ToolResult> {
  ...
  const ev = resp.data;
  // Calendar-branded card: event title + time row + attendees. source='calendar'
  // tells the client to show the Google Calendar header and skip the Scheduling
  // radio / Send button UI (this is a confirmation, not a proposal).
  const card: ScheduleCardJson = {
    type: 'schedule',
    title,
...
/* ─── Human labels for the task panel step list ─────────────────────── */
