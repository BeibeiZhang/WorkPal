...
// Kinds map 1:1 to the filter tabs on LibraryPage + ProjectPage Output.
// Keep in sync with the CHECK constraint in 0002_artifacts.sql.
...
export interface ArtifactContent {
  title: string;
  summary: string;
  // Template-shaped payload. For bay-area-weekend this is `{ categories: [{ key, label, items: [...] }] }`.
  // Kept as `unknown` at the store layer — the template registry owns the schema
  // and the ArtifactPage renders per-kind.
  body: unknown;
}
...
/** Idempotency check for recurring templates. Returns the existing row for
 *  the (templateId, weekKey) pair, or null. Ad-hoc rows have weekKey=NULL and
 *  are never deduped. */
export async function findArtifactByWeek(
  templateId: string,
  weekKey: string,
...
/** Insert a placeholder row with status='generating'. The pipeline updates it
 *  to 'ready' once content is populated, or to 'failed' on error. Returning
 *  the full row lets the caller surface id + slug immediately in the chat
 *  card so the user sees the skeleton before Tavily/OpenAI have returned. */
...
export async function markArtifactReady(id: string, patch: ArtifactReadyPatch): Promise<Artifact> {
  const { data, error } = await client()
    ...
    .update({
      status: 'ready',
      content_en: patch.contentEn,
      content_zh: patch.contentZh,
      cover_image_url: patch.coverImageUrl ?? null,
